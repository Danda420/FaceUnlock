...
public class ConfigureAndStartPreviewCallable extends CameraCallable {
    ...
    public void run() {
        try {
            ...
            if (DEBUG) Log.d(TAG, "Configuring Camera...");

            try {
                camera.setDisplayOrientation(90);
                if (DEBUG) Log.d(TAG, "Set Display Orientation: 90");
            } catch (Exception e) {
            ...
            try {
                ...
                if (bestSize != null) {
                    if (DEBUG) Log.d(TAG, "Setting Preview Size: " + bestSize.width + "x" + bestSize.height);
                ...
                if (params.isZoomSupported()) {
                    ...
                    if (DEBUG) Log.i(TAG, "Applying Crop-Zoom: " + targetZoom + "/" + maxZoom);
                }
            ...
            if (mSurface != null) {
                try {
                    ...
                    if (DEBUG) Log.d(TAG, "setPreviewSurface(Surface) called successfully");
                } catch (Exception e) {
            ...
            camera.startPreview();
            if (DEBUG) Log.d(TAG, "startPreview() called");
